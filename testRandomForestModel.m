...
clear afprintf('✅ Random Forest Model Test Complete!\n');
...
% Create test cases for each attack type
test_cases = struct();

% Normal traffic pattern
test_cases.normal = [
    0.4, 0.3, 0.3, 0.9, 0.2, 0.4, 0.7, ...  % Network topology
    0.2, 0.3, 0.1, 0.05, 0.1, 0, 0, ...      % Message content
    0.1, 0.3, 0.2, 0.8, 0.7, 0.1, ...        % Traffic patterns
    0.8, 0.5, 0.7, 0.6, 0.8, 0.9, ...        % Behavioral
    0.05, 0.05, 0.1, 0.95, 0.9, 0.95, ...    % Protocol
    0.1, 0.3, 0.2, 0.7, 0.1, 0.3, ...        % Resource
    0.8, 0.8, 0.8, 0.8, 0.8, 0.1             % Mesh-specific
];

% Flooding attack pattern
test_cases.flooding = [
    0.6, 0.2, 0.3, 0.8, 0.3, 0.3, 0.6, ...  % Network topology
    0.8, 0.4, 0.2, 0.08, 0.15, 0, 0, ...     % Message content (large messages)
    0.9, 0.8, 0.6, 0.2, 0.3, 0.8, ...        % Traffic patterns (high frequency)
    0.2, 0.3, 0.4, 0.4, 0.5, 0.6, ...        % Behavioral (low reputation)
    0.1, 0.1, 0.2, 0.9, 0.8, 0.9, ...        % Protocol
    0.7, 0.6, 0.5, 0.6, 0.2, 0.4, ...        % Resource (high impact)
    0.7, 0.7, 0.6, 0.7, 0.7, 0.2             % Mesh-specific
];

% Black hole attack pattern
test_cases.black_hole = [
    0.4, 0.3, 0.3, 0.4, 0.2, 0.4, 0.3, ...  % Network topology (poor hop reliability)
    0.3, 0.4, 0.1, 0.06, 0.1, 0, 0, ...      % Message content
    0.2, 0.3, 0.3, 0.7, 0.6, 0.2, ...        % Traffic patterns
    0.6, 0.4, 0.5, 0.5, 0.6, 0.7, ...        % Behavioral
    0.08, 0.05, 0.7, 0.9, 0.8, 0.9, ...      % Protocol (high routing anomaly)
    0.3, 0.4, 0.3, 0.6, 0.2, 0.4, ...        % Resource
    0.6, 0.6, 0.1, 0.6, 0.6, 0.3             % Mesh-specific (low forwarding)
];

% Spoofing attack pattern
test_cases.spoofing = [
    0.5, 0.3, 0.4, 0.8, 0.2, 0.4, 0.6, ...  % Network topology
    0.4, 0.6, 0.3, 0.08, 0.4, 2, 0, ...      % Message content (suspicious URLs)
    0.3, 0.4, 0.4, 0.6, 0.5, 0.3, ...        % Traffic patterns
    0.3, 0.4, 0.5, 0.5, 0.6, 0.7, ...        % Behavioral (low reputation)
    0.1, 0.08, 0.3, 0.7, 0.7, 0.6, ...       % Protocol (poor integrity)
    0.4, 0.5, 0.4, 0.6, 0.3, 0.4, ...        % Resource
    0.7, 0.7, 0.7, 0.7, 0.7, 0.3             % Mesh-specific
];
